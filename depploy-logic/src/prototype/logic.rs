...
pub async fn prototype_logic(cmd: Prototype) {
    match cmd {
        Prototype::Create { dir, debug } => {
            ...
            let res = send_creation_prototype(&cpt, &x, &cfg.prototype.unwrap().prototype_host).await.expect("Error");
        ...
        Prototype::Upload { dir, debug } => {
            ...
            upload_config(pcfg,&cfg.prototype.unwrap().prototype_host).await.expect("An error occured while uploading the config");
        }
    }
}